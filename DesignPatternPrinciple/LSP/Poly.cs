...
namespace DesignPatternPrinciple.LSP
{
    ...
    public class Poly
    {
        public static void Test()
        {
            ...
            instance.CommonMethod();//父类    普通方法由左边决定，编译时决定

            {
                typeof(ParentClass).GetMethod("CommonMethod").Invoke(instance, null);
                typeof(ChildClass).GetMethod("CommonMethod").Invoke(instance, null);//父类？ 还是子类？
            }
            {
                dynamic dInstance = new ChildClass();
                dInstance.CommonMethod();//子类:运行时决定的
            }
            //{
            //    var dInstance = new ChildClass();
            //}
            Console.WriteLine("下面是instance.VirtualMethod()");
            instance.VirtualMethod();//子类   虚方法由右边决定，运行时决定
            Console.WriteLine("下面是instance.AbstractMethod()");
            instance.AbstractMethod();//子类  抽象方法由右边决定，运行时决定
